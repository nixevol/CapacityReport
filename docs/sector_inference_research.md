...
sector（目标，66969 行）：`CGI, 扇区, 物理站, 区域, 制式, 频段, 带宽, 站型, 网络`
...
2. **D频 vs 3DMM**：同频点（2624.6 等）、同 PLMN(460-00)、同带宽(20M)、功率也重叠（18.2 两类都有）。**cellinfo 无法可靠区分**——3DMM 是 3D-MIMO 天线属性，非频点/名称可还原。逆推统一归 `TDD/D频`；现有 sector 中的 3DMM 因「不覆盖已有行」而保留，新增行只能判 D频（已知局限）。
...
- **700M/广电**：名称结构 `CBN-<枢纽>(<真实站址>)<码>` 或 `CBN-<站址><码>`。
  - 去码后取**最后一个括号内容**作为物理站；若该内容**以「共」开头**（共建标注，如 `(共…E-ZLW)`）则改取**括号前的外层**（去 `CBN-` 前缀）；无括号则取外层去 `CBN-`。
  - 取到的内容再删除其内部子括号（如 `(搬迁)(重建)`）。
  - 注意：`共和` 等地名（括号内容以「江门」开头、共字在中间）**不算共建**，仍取内层。
  - 例 `CBN-江门新会圭阳路(江门新会雍翠峰(搬迁))U-Z5H-0712` → `江门新会雍翠峰`。
  - 例 `CBN-江门台山通济公园U-Z5H-0713` → `江门台山通济公园`。
